...
## 轮 18 审查遗留(rev-fb20,2026-09-17;2 条 LOW,均不阻塞)

- **兜底分支的多模型偏移缺口(LOW,实测复现)**:模板态 + 容器段 `fields=[]`(容器记录被列空守卫剔除)+ 全网拓扑多模型时,兜底写构建期**局部** idx,未经 `modelIndex*10000` 偏移。不可达性:该判据是全局 options 判据,命中时**整个文件里不存在容器表**,引用本就悬空、无撞行可能。若要收口需把偏移量传入 finalizeContainerCrossRefs(增耦合),暂记。
- **设备表 `container_id` 在 `fields=[]` 边界下仍悬空带表名(LOW)**:与上一条同一边界;成员表已改裸 idx 后两处形态不对称,但 `container_id` 形态属轮 13 口径且用户未要求改。同上条一并待收口。
