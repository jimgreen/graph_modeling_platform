...
| 拖非成员落入容器范围(中心点判定) | **排斥:弹回容器矩形外**(最近边 + padding),不写归属 |
...
> 上表两行按新口径更新 —— 归属变更只在 Alt 拖动下发生,非 Alt 拖动落入容器 = 弹出。排斥仅对**拖动**生效:
> 非拖动入口(粘贴 / 模板落点 / 图元库放置 / SVG 导入 / 批量布局)仍「落入即移入」(judgeContainerMembership 的 `repelNonMembers` 开关区分)。
