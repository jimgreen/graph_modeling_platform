...
>
> **定位口径与边界(2026-09-17 复审降级,原「可唯一定位」为过度承诺):** 该引用定位到「表内行」,消费方仍需按行内 `name`(母线表另看 `realbs`)**复核**。原因与边界如下:
> - **最终行号:** 值在**序列化前定稿**产出,顺序 = 合并段 `idx` 重排**之后**(双绕组 `ACTransformer` 与三绕组 `ACTransfomer3` 同写 `trfm` 时,各设备的段内序号会在合并表里被重排成 1..N)—— 取段内序号会指向同表另一台设备。
> - **M1 退化形态不可辨:** 绑定设备无 E 段(理论不可达)时退化为**裸 idx**,与 `表名_idx` 形态无法从值本身区分;消费方按「含 `_` 才切分」判会误判此类值。
> - **M2 表名可含下划线:** 如 `dms_def_load_3`,切分必须按**最后一个**下划线。
...
- ~~模板模式下 `hasTemplateConfigValue && !definition` 的过滤照常生效 → 容器记录不输出~~ → **改为恒输出**;模板未定义容器段(或该类被类门控 `exportEnabled=false` 关掉)时:
  ...
  - ~~**列**走 `E_SECTION_COLUMNS["ACContainer"]` 兜底(idx/name/dev_type/is_gateway/bound_device_idx 五列),不按设备库定义重建字段~~ → **2026-09-17 轮 20 裁决取代:列集两态一致** —— 模板态同样按设备库定义重建字段(模板未定义该段时定义仍在、只被类门控置 `exportEnabled=false`,重建结果与无模板态同列,含 parent/p/q/u/i 等空值列,渲染为 0);`E_SECTION_COLUMNS` 五列兜底只在定义确实缺失(如合成 options)时生效
