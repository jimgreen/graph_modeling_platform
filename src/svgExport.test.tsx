...
describe("SVG export", () => {
  ...
  test("exports device labels and dynamic measurements with positioned refresh metadata", () => {
    ...
    expect(svg).toContain('class="export-measurement-group measurement-group" transform="translate(180 70)"');
    expect(svg).toContain('dev-id="load-export"');
    expect(svg).toContain('m-name="P主"');
    expect(svg).toContain('m-field="load-export.activePower"');
    expect(svg).toContain('m-unit="kW"');
    ...
    expect(svg).toContain('m-text="label"');
    expect(svg).toContain('m-text="value"');
    expect(svg).toContain('m-text="unit"');
    expect(svg).toContain('m-value="1"');
    expect(svg).toContain('m-type="activePower"');
    ...
    expect(valueText).toContain('m-value="1"');
    expect(valueText).toContain('m-id="m-active"');
    expect(valueText).toContain('m-name="P主"');
    ...
    const labelText = svg.match(/<text id="measurement_m-active_label" class="export-measurement-label measurement-label"[^>]*>P<\/text>/)?.[0] ?? "";
    const unitText = svg.match(/<text id="measurement_m-active_unit" class="export-measurement-unit measurement-unit"[^>]*>kW<\/text>/)?.[0] ?? "";
    expect(labelText).not.toContain('m-id="m-active"');
    expect(labelText).not.toContain('m-field=');
    expect(unitText).not.toContain('m-id="m-active"');
    expect(unitText).not.toContain('m-field=');
  ...
  test("keeps device labels in Text_Layer and measurements in Measurement_Layer instead of defs", () => {
    ...
    expect(measurementLayer).toContain('m-field="layered-text-load.activePower"');
