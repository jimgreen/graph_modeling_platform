...
export const terminalVbaseFallbackValue = (node: ModelNode, terminalIndex: number) => {
  if (node.kind === "ac-three-winding-transformer" || node.kind === "ac-three-winding-transformer-neutral") {
    return [node.params.high_vbase, node.params.medium_vbase, node.params.low_vbase, node.params.neutral_vbase][terminalIndex] ?? node.params.vbase ?? "";
  }
...
export const CUSTOM_DEVICE_DIALOG_DEFAULT_HEIGHT = 860;
...
export const PARAM_LABELS: Record<string, string> = {
  ...
  flow_min: "流量下限(Nm3/h)",
  soc: "SOC",
  soc_upper_limit: "SOC上限",
  soc_lower_limit: "SOC下限",
  i_max: "最大电流",
  frequency: "频率",
  ...
  rated_active_power: "额定有功",
  pv0: "pv0系数",
  pv1: "pv1系数",
  pv2: "pv2系数",
  rated_reactive_power: "额定无功",
  qv0: "qv0系数",
  qv1: "qv1系数",
  qv2: "qv2系数",
  resistance_pu: "电阻（标幺值）",
  reactance_pu: "电抗（标幺值）",
  half_charging_susceptance_pu: "半充电电纳（标幺值）",
  magnetizing_conductance_pu: "励磁电导（标幺值）",
  magnetizing_susceptance_pu: "励磁电纳（标幺值）",
  tap_ratio: "分接头档位/变比",
  high_rated_capacity: "高压侧额定容量",
  high_i_max: "高压侧最大电流",
  high_resistance_pu: "高压侧电阻（标幺值）",
  high_reactance_pu: "高压侧电抗（标幺值）",
  high_magnetizing_conductance_pu: "高压侧励磁电导（标幺值）",
  high_magnetizing_susceptance_pu: "高压侧励磁电纳（标幺值）",
  high_tap_ratio: "高压侧分接头档位/变比",
  medium_rated_capacity: "中压侧额定容量",
  medium_i_max: "中压侧最大电流",
  medium_resistance_pu: "中压侧电阻（标幺值）",
  medium_reactance_pu: "中压侧电抗（标幺值）",
  medium_magnetizing_conductance_pu: "中压侧励磁电导（标幺值）",
  medium_magnetizing_susceptance_pu: "中压侧励磁电纳（标幺值）",
  medium_tap_ratio: "中压侧分接头档位/变比",
  low_rated_capacity: "低压侧额定容量",
  low_i_max: "低压侧最大电流",
  low_resistance_pu: "低压侧电阻（标幺值）",
  low_reactance_pu: "低压侧电抗（标幺值）",
  low_magnetizing_conductance_pu: "低压侧励磁电导（标幺值）",
  low_magnetizing_susceptance_pu: "低压侧励磁电纳（标幺值）",
  low_tap_ratio: "低压侧分接头档位/变比",
  source_equivalent_resistance: "首端等值电阻",
  target_equivalent_resistance: "末端等值电阻",
  source_control_type: "首端控制类型",
  target_control_type: "末端控制类型",
  i_control_type: "首端控制类型",
  j_control_type: "末端控制类型",
  ac_control_type: "AC端控制类型",
  dc_control_type: "DC端控制类型",
  closed_status: "闭合状态",
  status: "运行状态",
  run_stat: "工作状态",
  backgroundImage: "背景图片",
  backgroundImageAssetId: "背景图片资产",
  backgroundImageFit: "显示方式",
  foregroundColor: "前景色",
...
export async function renameBackendImageFolder(folderId: string, name: string): Promise<ImageFolder> {
  return fetchBackendJson<ImageFolder>(
    apiPath(`/image-folders/${encodeURIComponent(folderId)}`),
    "重命名图片文件夹失败。",
...
export async function deleteBackendImageFolder(folderId: string): Promise<void> {
  await fetchBackendJson<{ ok?: boolean }>(apiPath(`/image-folders/${encodeURIComponent(folderId)}`), "删除图片文件夹失败。", {
    method: "DELETE"
  });
}

export async function fetchBackendImages(folderId = "root"): Promise<ImageAsset[]> {
  return fetchBackendJson<ImageAsset[]>(`${apiPath("/images")}?folderId=${encodeURIComponent(folderId)}`, "读取后台图片列表失败。");
}

export async function fetchAllBackendImages(): Promise<ImageAsset[]> {
  return fetchBackendJson<ImageAsset[]>(apiPath("/images"), "读取后台图片列表失败。");
}

export async function deleteBackendImageAsset(assetId: string): Promise<void> {
  await fetchBackendJson<{ ok?: boolean }>(apiPath(`/images/${encodeURIComponent(assetId)}`), "删除图片资源失败。", {
    method: "DELETE"
  });
}
