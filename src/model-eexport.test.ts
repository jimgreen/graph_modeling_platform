...
describe("交流容器 E 导出", () => {
  ...
  test("规格 B(裁决 2026-09-17 轮 15):段名按态分叉 —— 无模板态 ACContainerDev,模板态固定 container_dev", () => {
    ...
    expect(eOutputSectionName("ACContainerDev", definitionIndex, {})).toBe("ACContainerDev");
    expect(eOutputSectionName("ACContainerDev", definitionIndex, options)).toBe("container_dev");
    expect(eOutputSectionName("ACLoad", definitionIndex, {})).toBe("ACLoad");
  });
