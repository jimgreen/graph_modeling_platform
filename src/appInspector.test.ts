...
async function readStateIconDrawingSource() {
  // @ts-ignore - tests run in Node, while the app tsconfig intentionally stays browser-focused.
  const { readFile } = await import("node:fs/promises");
  return normalizeSourceText(await readFile(new URL("./stateIconDrawing.tsx", import.meta.url), "utf8"));
}
...
describe("graph inspector panel", () => {
  ...
  test("uses library group names for terminal energy dropdowns in device definition dialogs", async () => {
    ...
    const terminalLabelStart = source.indexOf("terminalLabels: customDeviceDraft.terminalLabels.slice(0, terminalTypes.length).map");
  ...
  test("edits enum parameter values and uses dropdown typical values in definition dialogs", async () => {
    ...
    expect(helperBlock).toContain("含义");
    expect(helperBlock).toContain("新增枚举值");
    expect(helperBlock).toContain("删除枚举值");
    expect(helperBlock).toContain("custom-param-enum-add-row");
    expect(helperBlock).toContain("custom-param-enum-spacer");
    expect(helperBlock).toContain('className={`custom-param-enum-values ${enumValueType === "number" ? "with-label" : ""}`}');
    ...
    expect(styles).toContain("grid-template-columns: minmax(58px, 1fr) max-content;");
    expect(styles).toContain(".custom-param-enum-values.with-label");
    expect(styles).toContain("display: contents;");
    expect(styles).toContain("max-content");
    expect(styles).toContain(".custom-param-enum-add-row");
  });
  ...
  test("places device measurement bindings inside the component definition dialog", async () => {
    ...
    const customTabsStart = dialogBlock.indexOf('role="tablist" aria-label="元件定义内容切换"');
    const customTabsEnd = dialogBlock.indexOf("<div className={`custom-device-tab-panel", customTabsStart);
    const customTabsBlock = dialogBlock.slice(customTabsStart, customTabsEnd);
    const customVisualStart = dialogBlock.indexOf('{visibleCustomDeviceDialogView === "terminals" || visibleCustomDeviceDialogView === "icon"');
    const customVisualEnd = dialogBlock.indexOf('visibleCustomDeviceDialogView === "parameters"', customVisualStart);
    const customVisualBlock = dialogBlock.slice(customVisualStart, customVisualEnd);
    const measurementPanelStart = source.indexOf("export function createRenderDeviceDefinitionMeasurementPanel(__appScope: Record<string, any>)");
    ...
    const customSaveBlock = source.slice(customSaveStart, customSaveEnd);
    const terminalDefinitionIndex = customTabsBlock.indexOf("端子定义");
    const iconDefinitionIndex = customTabsBlock.indexOf("图标定义");
    const parameterDefinitionIndex = customTabsBlock.indexOf("参数定义");
    const measurementDefinitionIndex = customTabsBlock.indexOf("量测定义");

    expect(source).toContain('type CustomDeviceDialogView = "terminals" | "icon" | "parameters" | "measurements";');
    expect(source).not.toContain("type CustomDeviceVisualView");
    expect(source).toContain('const [customDeviceDialogView, setCustomDeviceDialogView] = useState<CustomDeviceDialogView>("terminals");');
    ...
    expect(source).toContain("aria-label=\"搜索元件结构\"");
    expect(source).toContain("componentTypeDisplayParts(typeGroup.section)");
    expect(source).toContain("const [collapsedCustomComponentTreeTypes, setCollapsedCustomComponentTreeTypes] = useState<Set<string>>(new Set());");
    expect(source).toContain("initialCollapsedTypes={collapsedCustomComponentTreeTypes}");
    expect(source).toContain("onCollapseChange={handleTreeCollapseChange}");
    expect(source).toContain("const typeKey = `${group}::${typeGroup.section}`;");
    expect(source).toContain("const typeCollapsed = searchNeedle ? false : collapsedTypes.has(typeKey);");
    expect(source).toContain("className={`custom-component-tree-row type ${typeSelected ? \"active\" : \"\"}`}");
    expect(source).toContain("aria-expanded={!typeCollapsed}");
    expect(source).toContain("handleToggleType(group, typeGroup.section);");
    expect(source).toContain("!typeCollapsed && <div className=\"custom-component-tree-components\"");
    expect(dialogBlock).toContain("参数定义");
    expect(dialogBlock).toContain("量测定义");
    expect(customTabsBlock).toContain("端子定义");
    expect(customTabsBlock).toContain("图标定义");
    expect(customTabsBlock).toContain("{customDeviceHasTerminals && <button");
    ...
    expect(customVisualBlock).toContain('visibleCustomDeviceDialogView === "terminals" && <div className="custom-terminal-grid"');
    expect(customVisualBlock).toContain('visibleCustomDeviceDialogView === "icon" && renderStateVisualPager');
    expect(dialogBlock).toContain('visibleCustomDeviceDialogView === "parameters"');
    expect(dialogBlock).toContain('visibleCustomDeviceDialogView === "measurements"');
    expect(dialogBlock).toContain("renderDeviceDefinitionMeasurementPanel(customDeviceMeasurementTarget)");
    expect(source).toContain("type DeviceDefinitionMeasurementPanelTarget =");
    expect(source).toContain("const customDeviceMeasurementTarget: DeviceDefinitionMeasurementPanelTarget =");
    expect(source).toContain("export function createRenderDeviceDefinitionMeasurementPanel(__appScope: Record<string, any>)");
    expect(source).toContain("const renderDeviceDefinitionMeasurementPanel = createRenderDeviceDefinitionMeasurementPanel(__appScope);");
    expect(source).toContain("deviceDefinitionKeyForTemplate");
    expect(source).toContain("deviceDefinitionOverrideForTemplate");
    expect(source).toContain("const selectedKind = normalizeComponentTypeName(target.deviceKind);");
    ...
    expect(source).toContain("prepareMeasurementConfigDraft();");
    expect(source).toContain("saveCustomDeviceDefinitionDialog");
    expect(source).toContain("template.custom ? nextDraft : { ...nextDraft, error: \"\" }");
    expect(definitionSaveBlock).not.toContain("[selectedDefinitionTemplate.kind]: {");
    expect(measurementPanelBlock).toContain("量测定义会随右下角保存元件定义一并生效。");
    expect(measurementPanelBlock).not.toContain("保存量测定义");
    expect(measurementPanelBlock).toContain("<th>关联字段</th>");
    expect(measurementPanelBlock).toContain("associatedField");
    expect(measurementPanelBlock).toContain("onCommit={(nextValue) => updateMeasurementProfileItem(selectedKind, itemIndex, { associatedField: nextValue })}");
    expect(customSaveBlock).toContain("void saveMeasurementConfigDialog();");
    ...
    expect(styles).toContain(".custom-component-manager-tree");
  ...
  test("visually distinguishes built-in and custom library and device type options", async () => {
    const source = await readAppSource();
    const styles = await readStyles();
    const definitionSummaryStart = source.indexOf('className="device-definition-summary"');
    const definitionSummaryEnd = source.indexOf('<div className="device-definition-tabs"', definitionSummaryStart);
    const definitionSummaryBlock = source.slice(definitionSummaryStart, definitionSummaryEnd);
    const attributeLibraryIndex = definitionSummaryBlock.indexOf("<span>属性库</span>");
    const componentTypeIndex = definitionSummaryBlock.indexOf("<span>元件类型</span>");
    const componentNameIndex = definitionSummaryBlock.indexOf("<span>元件名称</span>");
    ...
    expect(definitionSummaryStart).toBeGreaterThanOrEqual(0);
    expect(attributeLibraryIndex).toBeGreaterThanOrEqual(0);
    expect(componentTypeIndex).toBeGreaterThan(attributeLibraryIndex);
    expect(componentNameIndex).toBeGreaterThan(componentTypeIndex);
    expect(definitionSummaryBlock).not.toContain("<span>默认尺寸</span>");
    expect(definitionSummaryBlock).not.toContain("<span>定义状态</span>");
  ...
  test("allows custom component definition dialogs to be manually moved and resized", async () => {
    const source = await readAppSource();
    const styles = await readStyles();
    const definitionDialogStart = source.indexOf("{deviceDefinitionDialogOpen && (");
    const definitionDialogEnd = source.indexOf("{customDeviceDialogOpen && (", definitionDialogStart);
    ...
    expect(source).toContain("type DeviceLibraryDialogKind = \"definition\" | \"custom\" | \"measurementConfig\";");
    expect(source).toContain("type DeviceLibraryDialogLayouts =");
    expect(source).toContain("type DeviceLibraryDialogPointerState =");
    expect(source).toContain("const [deviceLibraryDialogLayouts, setDeviceLibraryDialogLayouts]");
    expect(source).toContain("const [deviceLibraryDialogDrag, setDeviceLibraryDialogDrag]");
    expect(source).toContain("const [deviceLibraryDialogResize, setDeviceLibraryDialogResize]");
    expect(source).toContain("const deviceDefinitionDialogRef = useRef<HTMLElement | null>(null);");
    expect(source).toContain("const customDeviceDialogRef = useRef<HTMLElement | null>(null);");
    expect(source).toContain("const measurementConfigDialogRef = useRef<HTMLElement | null>(null);");
    expect(source).toContain("function clampDeviceLibraryDialogLayout");
    expect(source).toContain("const currentDeviceLibraryDialogRect =");
    expect(source).toContain("const startDeviceLibraryDialogDrag =");
    expect(source).toContain("const startDeviceLibraryDialogResize =");
    expect(source).toContain("const finishDeviceLibraryDialogPointerOperation =");
    expect(source).toContain("setDeviceLibraryDialogDrag(null);");
    expect(source).toContain("setDeviceLibraryDialogResize(null);");
    expect(source).toContain("event.buttons === 0");
    expect(source).toContain("window.addEventListener(\"pointermove\", handlePointerMove, true);");
    expect(source).toContain("window.addEventListener(\"pointerup\", handlePointerUp, true);");
    expect(definitionDialogBlock).toContain("ref={deviceDefinitionDialogRef}");
    expect(definitionDialogBlock).toContain("className={`device-definition-dialog${deviceLibraryDialogLayouts.definition ? \" floating\" : \"\"}`}");
    expect(definitionDialogBlock).toContain("style={deviceLibraryDialogStyle(\"definition\")}");
    expect(definitionDialogBlock).toContain("onPointerDown={stopDeviceLibraryDialogEvent}");
    expect(definitionDialogBlock).toContain("onPointerCancel={stopDeviceLibraryDialogEvent}");
    expect(definitionDialogBlock).toContain("onLostPointerCapture={stopDeviceLibraryDialogEvent}");
    expect(definitionDialogBlock).toContain("className=\"device-library-dialog-title\"");
    expect(definitionDialogBlock).toContain("startDeviceLibraryDialogDrag(\"definition\"");
    expect(definitionDialogBlock).toContain("className=\"device-library-dialog-resize\"");
    expect(definitionDialogBlock).toContain("startDeviceLibraryDialogResize(\"definition\"");
    expect(customDialogBlock).toContain("ref={customDeviceDialogRef}");
    expect(customDialogBlock).toContain("className={`custom-device-dialog${deviceLibraryDialogLayouts.custom ? \" floating\" : \"\"}`}");
    expect(customDialogBlock).toContain("style={deviceLibraryDialogStyle(\"custom\")}");
    expect(customDialogBlock).toContain("onPointerDown={stopDeviceLibraryDialogEvent}");
    expect(customDialogBlock).toContain("onPointerCancel={stopDeviceLibraryDialogEvent}");
    expect(customDialogBlock).toContain("onLostPointerCapture={stopDeviceLibraryDialogEvent}");
    expect(customDialogBlock).toContain("className=\"device-library-dialog-title\"");
    expect(customDialogBlock).toContain("startDeviceLibraryDialogDrag(\"custom\"");
    expect(customDialogBlock).toContain("className=\"device-library-dialog-resize\"");
    expect(customDialogBlock).toContain("startDeviceLibraryDialogResize(\"custom\"");
    expect(measurementConfigDialogBlock).toContain("ref={measurementConfigDialogRef}");
    expect(measurementConfigDialogBlock).toContain("className={`measurement-config-dialog${deviceLibraryDialogLayouts.measurementConfig ? \" floating\" : \"\"}`}");
    expect(measurementConfigDialogBlock).toContain("style={deviceLibraryDialogStyle(\"measurementConfig\")}");
    expect(measurementConfigDialogBlock).toContain("onPointerDown={stopDeviceLibraryDialogEvent}");
    expect(measurementConfigDialogBlock).toContain("onPointerCancel={stopDeviceLibraryDialogEvent}");
    expect(measurementConfigDialogBlock).toContain("onLostPointerCapture={stopDeviceLibraryDialogEvent}");
    expect(measurementConfigDialogBlock).toContain("className=\"device-library-dialog-title\"");
    expect(measurementConfigDialogBlock).toContain("startDeviceLibraryDialogDrag(\"measurementConfig\"");
    expect(measurementConfigDialogBlock).toContain("className=\"device-library-dialog-resize\"");
    expect(measurementConfigDialogBlock).toContain("startDeviceLibraryDialogResize(\"measurementConfig\"");
    expect(styles).toContain(".app-shell.device-library-dialog-moving");
    expect(styles).toContain(".custom-device-dialog.floating,");
    expect(styles).toContain(".device-definition-dialog.floating");
    expect(styles).toContain(".measurement-config-dialog.floating");
    expect(floatingBlock).toContain("position: fixed");
    expect(titleBlock).toContain("cursor: move");
    expect(titleBlock).toContain("touch-action: none");
    expect(resizeBlock).toContain("cursor: nwse-resize");
  ...
  test("shows custom device terminal guide lines without ratio labels", async () => {
    const source = await readAppSource();
    const styles = await readStyles();

    expect(source).toContain("className={`custom-device-terminal-guide ${active ? \"active\" : \"\"}`}");
    expect(source).not.toContain("custom-device-terminal-guide-label");
    expect(styles).toContain(".custom-device-terminal-guide");
    expect(styles).not.toContain(".custom-device-terminal-guide-label");
  });

  test("hides custom device terminal definition tab when terminal count is zero", async () => {
    const source = await readAppSource();
    const dialogStart = source.indexOf("{customDeviceDialogOpen && (");
    const dialogEnd = source.indexOf("{stateIconDrawingDialog && (", dialogStart);
    const dialogBlock = source.slice(dialogStart, dialogEnd);
    ...
    const customTabsEnd = dialogBlock.indexOf("<div className={`custom-device-tab-panel", customTabsStart);
    const customTabsBlock = dialogBlock.slice(customTabsStart, customTabsEnd);
    const updateCountStart = source.indexOf("export function createUpdateCustomDraftTerminalCount");
    const updateCountEnd = source.indexOf("export function createChooseCustomDeviceBackground", updateCountStart);
    const updateCountBlock = source.slice(updateCountStart, updateCountEnd);

    expect(source).toContain("const customDeviceHasTerminals = customDeviceDraft.terminalCount > 0;");
    expect(source).toContain('customDeviceDialogView === "terminals" && !customDeviceHasTerminals ? "icon" : customDeviceDialogView');
    expect(customTabsBlock).toContain("{customDeviceHasTerminals && <button");
    expect(customTabsBlock).toContain('className={visibleCustomDeviceDialogView === "terminals" ? "active" : ""}');
    expect(dialogBlock).toContain('custom-device-tab-panel-${visibleCustomDeviceDialogView}');
    expect(updateCountBlock).toContain("if (count === 0) {");
    expect(updateCountBlock).toContain('setCustomDeviceDialogView("icon");');
    expect(source).toContain('setCustomDeviceDialogView(nextDraft.terminalCount > 0 ? "terminals" : "icon");');
  });
  ...
  test("exposes dynamic measurement configuration and per-device measurement operations", async () => {
    ...
    expect(libraryDefinitionActionsBlock).toContain("元件定义");
    expect(libraryDefinitionActionsBlock).toContain("openDeviceDefinitionDialog");
    expect(libraryDefinitionActionsBlock).not.toContain("新建元件");
    expect(libraryDefinitionActionsBlock).not.toContain("修改元件");
    ...
    expect(source).toContain("<BufferedTextInput");
    expect(source).toContain("name: nextValue");
  ...
  test("edits template icons and terminal anchors from the device definition dialog", async () => {
    ...
    expect(dialogBlock).toContain("端子定义");
    expect(dialogBlock).not.toContain("图标/端子");
  ...
  test("edits status pages inside visual icon editors while sharing size and terminals", async () => {
    ...
    const customDialogEnd = source.indexOf("{imageTarget && (", customDialogStart);
    const customDialogBlock = source.slice(customDialogStart, customDialogEnd);
    const visualPagerStart = source.indexOf("export function createRenderStateVisualPager");
    const visualPagerEnd = source.indexOf("export function createRenderDeviceDefinitionVisualPanel", visualPagerStart);
    const visualPagerBlock = source.slice(visualPagerStart, visualPagerEnd);
    const definitionVisualStart = source.indexOf("export function createRenderDeviceDefinitionVisualPanel");
    ...
    const definitionVisualSaveStart = source.indexOf("export function createSaveDeviceDefinitionVisualDraft");
    const definitionVisualSaveEnd = source.indexOf("export function createSaveDeviceDefinitionDraft", definitionVisualSaveStart);
    ...
    expect(visualPagerBlock.indexOf("删除状态")).toBeLessThan(visualPagerBlock.indexOf("device-state-page-fields"));
    expect(visualPagerBlock).not.toContain("状态图标");
    expect(visualPagerBlock).not.toContain("状态图片");
    expect(visualPagerBlock).not.toContain("状态图标绘制");
    expect(visualPagerBlock).not.toContain("绘制图标");
    expect(visualPagerBlock).not.toContain("状态文字");
    expect(visualPagerBlock).not.toContain("主颜色");
    expect(visualPagerBlock).not.toContain("填充色");
    expect(visualPagerBlock).not.toContain("边框色");
    expect(visualPagerBlock).not.toContain("文字色");
    expect(visualPagerBlock).toContain("state-icon-drawing-inline");
    expect(visualPagerBlock).toContain("state-icon-drawing-layout");
    expect(visualPagerBlock).toContain("state-icon-drawing-canvas");
    expect(visualPagerBlock).toContain("添加图案");
    expect(visualPagerBlock).toContain("导入SVG");
    expect(visualPagerBlock).toContain("导入图片");
    expect(source).not.toContain("state-icon-drawing-backdrop");
    expect(source).not.toContain("state-icon-drawing-dialog");
    expect(source).not.toContain("应用到状态图标");
  ...
  test("stretches custom device editor tab pages to the available dialog height", async () => {
    ...
    const dialogBlock = styles.slice(dialogStart, dialogEnd);
    ...
    const editorEnd = styles.indexOf(".custom-device-dialog-footer", editorStart);
    const editorBlock = styles.slice(editorStart, editorEnd);
    const tabPanelStart = styles.indexOf(".custom-device-tab-panel {");
    const tabPanelEnd = styles.indexOf(".custom-device-dialog-footer", tabPanelStart);
    const tabPanelBlock = styles.slice(tabPanelStart, tabPanelEnd);
    ...
    const terminalPanelBlock = cssRuleBlock(styles, ".custom-device-tab-panel-terminals {");
    const terminalGridBlock = cssRuleBlock(styles, ".custom-device-tab-panel-terminals > .custom-terminal-grid {");
    const terminalAnchorInputsBlock = cssRuleBlock(styles, ".custom-device-tab-panel-terminals .custom-terminal-anchor-inputs {");
    const terminalPreviewBlock = cssRuleBlock(styles, ".custom-device-tab-panel-terminals > .custom-device-preview {");
    const terminalPreviewStageBlock = cssRuleBlock(styles, ".custom-device-tab-panel-terminals > .custom-device-preview .custom-device-preview-stage {");
    const terminalPreviewSvgBlock = cssRuleBlock(styles, ".custom-device-tab-panel-terminals > .custom-device-preview .custom-device-anchor-preview {");
    const iconPanelBlock = cssRuleBlock(styles, ".custom-device-tab-panel-icon {");
    const iconStatePagerBlock = cssRuleBlock(styles, ".custom-device-tab-panel-icon > .device-state-pager {");
    const iconPreviewBlock = cssRuleBlock(styles, ".custom-device-tab-panel-icon > .custom-device-preview {");
    const iconPreviewStageBlock = cssRuleBlock(styles, ".custom-device-tab-panel-icon > .custom-device-preview .custom-device-preview-stage {");
    const iconPreviewSvgBlock = cssRuleBlock(styles, ".custom-device-tab-panel-icon > .custom-device-preview .custom-device-anchor-preview {");
    const stateIconLibraryBlock = cssRuleBlock(styles, ".device-state-pager > .state-icon-drawing-library {");
    const stateIconToolButtonBlock = cssRuleBlock(styles, ".state-icon-drawing-library .state-icon-import-button,\n.state-icon-drawing-library .state-icon-tool-button");
    const stateIconCanvasBlock = cssRuleBlock(styles, ".state-icon-drawing-canvas {");
    const stateIconCanvasSvgBlock = cssRuleBlock(styles, ".state-icon-drawing-canvas svg {");
    ...
    expect(dialogBlock).toContain("height: min(760px, calc(100vh - 48px));");
    expect(dialogBlock).toContain("overflow: hidden;");
    expect(layoutBlock).toContain("flex: 1 1 auto;");
    ...
    expect(source).toContain('className={`custom-device-tab-panel custom-device-tab-panel-${visibleCustomDeviceDialogView}`}');
    ...
    expect(source).not.toContain("customDeviceIconPreviewMargin");
    expect(source).toContain("const customDeviceTerminalPreviewViewBox = {");
    expect(source).toContain("customDeviceTerminalPreviewBounds.maxX - customDeviceTerminalPreviewBounds.minX + customDeviceTerminalPreviewPadding * 2");
    expect(source).toContain("formatSvgNumber(customDeviceTerminalPreviewViewBox.x)");
    expect(parameterPanelBlock).toContain("grid-template-rows: minmax(0, 1fr) auto;");
    expect(terminalPanelBlock).toContain("grid-template-rows: auto minmax(0, 1fr);");
    expect(terminalPanelBlock).toContain("overflow: hidden;");
    expect(source).toContain('"--custom-terminal-count": Math.max(1, customDeviceDraft.terminalCount)');
    expect(terminalGridBlock).toContain("order: 1;");
    expect(terminalGridBlock).toContain("grid-template-columns: repeat(var(--custom-terminal-count, 1), minmax(0, 1fr));");
    expect(terminalGridBlock).toContain("overflow: hidden;");
    expect(terminalAnchorInputsBlock).toContain("grid-template-columns: auto minmax(42px, 1fr) auto minmax(42px, 1fr);");
    expect(terminalAnchorInputsBlock).toContain("gap: 3px;");
    expect(terminalPreviewBlock).toContain("order: 2;");
    expect(terminalPreviewBlock).toContain("height: 100%;");
    expect(terminalPreviewBlock).toContain("overflow: hidden;");
    expect(terminalPreviewStageBlock).toContain("height: 100%;");
    expect(terminalPreviewStageBlock).toContain("min-height: 0;");
    expect(terminalPreviewSvgBlock).toContain("height: 100%;");
    expect(terminalPreviewSvgBlock).toContain("min-height: 0;");
    expect(iconPanelBlock).toContain("grid-template-rows: minmax(0, 1fr);");
    expect(iconPanelBlock).toContain("overflow: hidden;");
    expect(iconStatePagerBlock).toContain("height: 100%;");
    expect(iconStatePagerBlock).toContain("overflow: hidden;");
    expect(iconStatePagerBlock).toContain("grid-template-rows: auto auto auto minmax(0, 1fr) auto auto;");
    expect(stateIconLibraryBlock).toContain("grid-template-columns: auto auto minmax(0, 1fr);");
    expect(stateIconLibraryBlock).toContain("padding: 6px 8px;");
    expect(stateIconToolButtonBlock).toContain("width: 30px;");
    expect(stateIconToolButtonBlock).toContain("min-width: 30px;");
    expect(source).toContain('className="state-icon-tool-button"');
    expect(source).toContain('className="state-icon-import-button"');
    expect(source).toContain('aria-label={stateVisualShapeLabel(kind)}');
    expect(source).toContain('title={stateVisualShapeLabel(kind)}');
    expect(source).toContain('renderStateIconDrawingToolIcon(kind)');
    expect(source).toContain('renderStateIconDrawingImportIcon("svg")');
    expect(source).toContain('renderStateIconDrawingImportIcon("image")');
    expect(stateIconCanvasBlock).toContain("min-height: 0;");
    expect(stateIconCanvasBlock).toContain("height: 100%;");
    expect(stateIconCanvasSvgBlock).toContain("height: 100%;");
    expect(stateIconCanvasSvgBlock).toContain("max-height: none;");
    expect(iconPreviewBlock).toContain("height: 100%;");
    expect(iconPreviewBlock).toContain("overflow: hidden;");
    expect(iconPreviewStageBlock).toContain("height: 100%;");
    expect(iconPreviewStageBlock).toContain("min-height: 0;");
    expect(iconPreviewSvgBlock).toContain("height: 100%;");
    expect(iconPreviewSvgBlock).toContain("min-height: 0;");
  ...
  test("keeps default status page structural edits separate from state style edits", async () => {
    const source = await readAppSource();
    const stateIconDrawingSource = await readStateIconDrawingSource();
    const drawingSource = `${source}\n${stateIconDrawingSource}`;
    const styles = await readStyles();
    const visualPagerStart = source.indexOf("export function createRenderStateVisualPager");
    const visualPagerEnd = source.indexOf("export function createRenderDeviceDefinitionVisualPanel", visualPagerStart);
    const visualPagerBlock = source.slice(visualPagerStart, visualPagerEnd);
    ...
    const customDialogEnd = source.indexOf("{imageTarget && (", customDialogStart);
    const customDialogBlock = source.slice(customDialogStart, customDialogEnd);
    const definitionStateSaveStart = source.indexOf("export function createSaveDeviceDefinitionStateVisualDraft");
    const definitionStateSaveEnd = source.indexOf("export function createSaveDeviceDefinitionVisualDraft", definitionStateSaveStart);
    const definitionStateSaveBlock = source.slice(definitionStateSaveStart, definitionStateSaveEnd);
    const addDefinitionStateStart = source.indexOf("export function createAddDefinitionStateDraftRow");
    const addDefinitionStateEnd = source.indexOf("export function createDeleteDefinitionStateDraftRow", addDefinitionStateStart);
    const addDefinitionStateBlock = source.slice(addDefinitionStateStart, addDefinitionStateEnd);
    const addCustomStateStart = source.indexOf("export function createAddCustomDeviceStateDraftRow");
    const addCustomStateEnd = source.indexOf("export function createDeleteCustomDeviceStateDraftRow", addCustomStateStart);
    ...
    expect(source).toContain("DEFAULT_STATE_PAGE_ID");
    expect(source).toContain("createStateDraftRowFromDefaultVisual");
    ...
    expect(drawingSource).toContain("type StateIconDrawingElement =");
    ...
    expect(drawingSource).toContain('"imported-svg"');
    expect(drawingSource).toContain('"image"');
    expect(drawingSource).toContain("function visibleStateIconColor");
    expect(drawingSource).toContain('const stroke = visibleStateIconColor("#2563eb", row.strokeColor, row.color);');
    expect(drawingSource).toContain("function createStateIconDrawingElement");
    expect(drawingSource).toContain('const strokeColor = visibleStateIconColor("#2563eb", row?.strokeColor, row?.color);');
    expect(drawingSource).not.toContain('strokeColor: row?.strokeColor.trim() || row?.color.trim() || "#2563eb"');
    expect(drawingSource).toContain("function createImportedStateIconElement");
    expect(drawingSource).toContain("function svgSourceFromDataUrl");
    expect(drawingSource).toContain("function parseStateIconSvgSource");
    expect(drawingSource).toContain("function stateIconSvgNodeToReact");
    expect(drawingSource).toContain("function stateIconSvgSourceToReactNodes");
    expect(drawingSource).toContain("function parseSvgStyleAttribute");
    expect(drawingSource).toContain("return <polygon key={key} {...props} />;");
    expect(drawingSource).not.toContain("const props = { ...stateIconSvgReactAttributes(element), key };");
    expect(drawingSource).toContain("function createEditableStateIconElementsFromSvgSource");
    expect(drawingSource).toContain("function createStateIconDrawingInitialElements");
    expect(drawingSource).toContain("function svgSourceToDataUrl");
    expect(drawingSource).toContain("function stateIconDrawingSvgElementMarkup");
    expect(drawingSource).toContain("function stateIconDrawingToImage");
    expect(drawingSource).toContain("function stateIconDrawingElementPreviewImage");
    expect(drawingSource).toContain("function stateIconDrawingElementPreviewNode");
    expect(drawingSource).toContain("clip-path");
    ...
    expect(source).toContain("export function createChooseStateVisualImage");
    expect(source).toContain("export function createChooseStateIconDrawingImport");
    expect(source).toContain("const importedElements = isSvg");
    expect(source).toContain("export function createStartStateIconDrawingDrag");
    expect(source).toContain("export function createDragStateIconDrawingSelection");
    expect(source).toContain("export function createDeleteSelectedStateIconDrawingElements");
    expect(source).toContain("export function createOpenStateIconDrawingDialog");
    expect(source).toContain("const initial = createStateIconDrawingInitialElements(row, imageAssets);");
    expect(source).toContain("export function createApplyStateIconDrawingDialog");
    expect(source).toContain("stateVisualImageInputRef");
    expect(source).toContain("stateIconDrawingInlineTarget");
    expect(source).toContain("stateIconDrawingInlineImage");
    expect(source).toContain("updateDefinitionStateDraftRow(stateIconDrawingDialog.target.rowId, stateIconDrawingInlinePatch)");
    expect(source).toContain("updateCustomDeviceStateDraftRow(stateIconDrawingDialog.target.rowId, stateIconDrawingInlinePatch)");
    expect(visualPagerBlock).not.toContain("上传图形");
    expect(visualPagerBlock).not.toContain("绘制图标");
    expect(visualPagerBlock).not.toContain("handlers.drawStateIcon");
    expect(source).not.toContain("状态图标绘制器");
    ...
    expect(customDialogBlock).toContain('customDefaultStateSelected && visibleCustomDeviceDialogView === "terminals" && <div className="custom-device-preview">');
    expect(customDialogBlock).not.toContain('visibleCustomDeviceDialogView === "icon" && customDefaultStateSelected && <div className="custom-device-preview">');
    expect(customDialogBlock).not.toContain("SVG/图片图标");
    ...
    expect(styles).not.toContain(".device-state-shape-tools");
    expect(styles).toContain(".state-icon-drawing-inline");
    expect(styles).not.toContain(".state-icon-drawing-dialog");
